...
function App() {
  ...
  createUserWithEmailAndPassword(auth,loginData.email,loginData.password)
    .then((response)=>console.log(response)).catch((err)=>alert(err.message));
  }

  const handleLogin=()=>{
    signInWithEmailAndPassword(auth,loginData.email,loginData.password)
    .then((response)=>{console.log(response);console.log('name: ',response.user.name, 'Token: ',response.user.accessToken)}).catch((err)=>alert(err.message));
  }
